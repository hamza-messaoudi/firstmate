...
test_matrix_herdr_halfblock_rule_bounds_bare_wrap() {
  ...
  local screen plain out typed_screen
  ...
  typed_screen=$'transcript\n ▄▄▄▄▄▄▄▄\n'"  ${ESC_LOCAL}[2m→ ${ESC_LOCAL}[0m${ESC_LOCAL}[38;2;224;222;244mAdd a follow-up${ESC_LOCAL}[0m"$'\n ▀▀▀▀▀▀▀▀\n  Cursor Grok 4.5 High · 6.7%   Run Everything\n  ~/wt · 64cdd3a'
  out=$(fm_composer_classify_screen "$CAPS_STYLED" "$typed_screen")
  [ "$out" = pending ] \
    || fail "typed text inside herdr half-block rules must stay pending, got '$out'"
