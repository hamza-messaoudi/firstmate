...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`), scout (optionally `--plan` for a structured implementation-plan report), secondmate-charter, and Herdr-lab briefs, with Captain's intent and Firstmate spec subsections on ship/scout |
| `fm-plan-dispatch.sh`    | Convert an approved plan report into component ship tasks and briefs, then print tier-grouped spawn suggestions |
